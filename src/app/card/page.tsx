    ...
    <div className="min-h-screen bg-[#0A0E1A] pb-24">
      ...
      <div className="p-4 space-y-6">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#161B22] to-[#0D1117] border border-[#21262D]"
        >
          {/* 배경 장식 */}
          <div className="absolute top-0 right-0 w-40 h-40 bg-gradient-to-br from-[#00E5FF]/10 to-transparent rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-[#7C4DFF]/10 to-transparent rounded-full blur-3xl" />
          ...
          <div className="relative p-6">
            ...
            {myCard.keywords && myCard.keywords.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {myCard.keywords.slice(0, 5).map((keyword, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1 text-xs font-medium rounded-full bg-[#00E5FF]/10 text-[#00E5FF] border border-[#00E5FF]/20"
                  >
                    {keyword}
                  </span>
                ))}
            ...
            {/* QR 코드 섹션 */}
            <div className="flex flex-col items-center py-6 border-t border-[#21262D]">
              {isPremium ? (
                // 프리미엄 사용자: QR 코드 표시
                qrDataUrl ? (
                  <motion.img
                  ...
                  <div className="w-48 h-48 rounded-xl bg-[#161B22] flex items-center justify-center">
                ...
                <button
                  ...
                  className="relative w-48 h-48 rounded-xl bg-[#161B22] flex flex-col items-center justify-center group overflow-hidden"
                >
                  {/* 블러된 QR 코드 배경 */}
                  <div className="absolute inset-0 opacity-30 blur-sm">
                    <QrCode size={120} className="text-[#484F58] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
                  </div>
                  {/* 잠금 오버레이 */}
                  <div className="relative z-10 flex flex-col items-center">
                    ...
                    <span className="text-[#8B949E] text-xs mt-1">탭하여 업그레이드</span>
                  </div>
                </button>
              )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <motion.button
            ...
            className={`flex flex-col items-center gap-2 p-4 rounded-xl bg-[#161B22] border border-[#21262D] relative ${!isPremium ? 'opacity-60' : ''}`}
          >
            <Share2 size={24} className="text-[#00E5FF]" />
          ...
          >
            {copied ? (
              ...
              <Copy size={24} className="text-[#00E5FF]" />
            )}
          ...
          <motion.button
            ...
            className={`flex flex-col items-center gap-2 p-4 rounded-xl bg-[#161B22] border border-[#21262D] relative ${!isPremium ? 'opacity-60' : ''}`}
          >
            <Download size={24} className="text-[#00E5FF]" />
        ...
        {!isPremium && (
          ...
          >
            ...
            <div className="flex-1 text-left">
              ...
              <p className="text-[#8B949E] text-sm">QR 명함 생성 및 공유 기능을 이용하세요</p>
            </div>
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-4 p-4 rounded-xl border transition-all ${
                        isSelected
                          ...
                          : 'bg-[#0D1117] border-[#21262D] hover:border-[#484F58]'
                      }`}
                    >
                      <div className={`p-2 rounded-lg ${isSelected ? 'bg-[#00E5FF]/20' : 'bg-[#21262D]'}`}>
                        <Icon size={20} className={isSelected ? 'text-[#00E5FF]' : 'text-[#8B949E]'} />
                      </div>
                      <div className="flex-1 text-left">
        ...
        {showUpgradeModal && (
          ...
          >
            <motion.div
              ...
              transition={{ type: 'spring', damping: 25, stiffness: 300 }}
              onClick={(e) => e.stopPropagation()}
              className="w-full max-h-[90vh] bg-[#161B22] rounded-t-3xl border-t border-[#21262D] overflow-hidden"
            >
              {/* 헤더 */}
              <div className="relative px-6 pt-6 pb-4">
                ...
                <button
                  onClick={() => setShowUpgradeModal(false)}
                  className="absolute top-6 right-4 p-2 rounded-full hover:bg-[#21262D] transition-colors"
                >
                ...
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-bold text-white mb-2">NEXUS 프리미엄</h2>
                  <p className="text-[#8B949E] text-sm">나만의 QR 명함으로 네트워킹을 시작하세요</p>
                </div>
              </div>
              ...
              <div className="px-6 py-4">
                <div className="grid grid-cols-2 gap-3">
                  <button className="p-4 rounded-xl border-2 border-[#00E5FF] bg-[#00E5FF]/10 text-left">
                    <div className="text-xs text-[#00E5FF] font-medium mb-1">월간 구독</div>
                    <div className="text-2xl font-bold text-white">₩{PREMIUM_PRICE.monthly.toLocaleString()}</div>
                    <div className="text-xs text-[#8B949E]">매월 결제</div>
                  </button>
                  <button className="p-4 rounded-xl border border-[#21262D] bg-[#0D1117] text-left relative">
                    <div className="absolute -top-2 right-2 px-2 py-0.5 bg-[#00E676] text-black text-[10px] font-bold rounded-full">
                      33% 할인
                    </div>
                    <div className="text-xs text-[#8B949E] font-medium mb-1">연간 구독</div>
                    <div className="text-2xl font-bold text-white">₩{PREMIUM_PRICE.yearly.toLocaleString()}</div>
                    <div className="text-xs text-[#8B949E]">연 1회 결제</div>
                  </button>
                </div>
              </div>
                  ...
                  {PREMIUM_FEATURES.map((feature, idx) => (
                    <div key={idx} className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[#8B949E] text-xs">{feature.description}</p>
                      </div>
                    </div>
                  ))}
                </div>

                <h3 className="text-sm font-semibold text-[#8B949E] mt-6 mb-3">무료 기능 (모든 사용자)</h3>
                <div className="space-y-2">
                  {FREE_FEATURES.map((feature, idx) => (
                    <div key={idx} className="flex items-center gap-3">
                      <Check size={14} className="text-[#00E676]" />
                      <span className="text-[#8B949E] text-sm">{feature.title}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* CTA 버튼 */}
              <div className="px-6 py-6 border-t border-[#21262D] bg-[#0D1117]">
                <button
                  onClick={() => {
                    // TODO: 실제 결제 연동
                    alert('결제 기능은 준비 중입니다.');
                  }}
                  className="w-full py-4 rounded-xl bg-gradient-to-r from-[#FFD700] to-[#FFA500] text-black font-bold text-lg flex items-center justify-center gap-2"
                >
                  <Crown size={20} />
                  프리미엄 시작하기
                </button>
