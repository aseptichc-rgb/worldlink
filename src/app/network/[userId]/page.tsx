    ...
    <div className="min-h-screen bg-[#FAFBFC] pb-24">
      ...
      <div className="sticky top-0 z-30 bg-[#FAFBFC]/80 backdrop-blur-sm border-b border-[#E2E8F0]">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-[#1A1A2E]">{targetUser.name}의 인맥</h1>
      ...
      <div className="p-4 space-y-6">
        ...
        <motion.div
          ...
          className="p-6 rounded-xl bg-[#F1F3F5] border border-[#E2E8F0]"
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <h2 className="text-xl font-bold text-[#1A1A2E]">{targetUser.name}</h2>
              {targetUser.position && (
          ...
          {targetUser.keywords && targetUser.keywords.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-[#E2E8F0]">
              {targetUser.keywords.map((keyword, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1 text-sm rounded-full bg-[#2563EB]/10 text-[#2563EB]"
                >
                  {keyword}
                </span>
              ))}
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t border-[#E2E8F0]">
            ...
            <span className="text-[#1A1A2E] font-medium">{connections.length}명</span>
            <span className="text-[#64748B]">의 인맥</span>
          </div>
        </motion.div>
        ...
        {isLocked ? (
          <div className="flex flex-col items-center justify-center py-16">
            <div className="w-20 h-20 rounded-full bg-[#F1F3F5] flex items-center justify-center mb-4">
              <Lock size={32} className="text-[#94A3B8]" />
            </div>
            <p className="text-[#1A1A2E] font-medium mb-2">비공개 인맥</p>
          ...
          <>
            <div className="flex items-center justify-between">
              <h3 className="text-[#1A1A2E] font-semibold">인맥 목록</h3>
              <span className="text-base text-[#64748B]">{connections.length}명</span>
            </div>
            ...
            <div className="space-y-3">
              {connections.map((connection, index) => (
                <motion.div
                  ...
                  className="p-4 rounded-xl bg-[#F1F3F5] border border-[#E2E8F0]"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-[#1A1A2E]">{connection.name}</h4>
                      ...
                      {connection.keywords && connection.keywords.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-2">
                          {connection.keywords.slice(0, 2).map((keyword, idx) => (
                            <span
                              key={idx}
                              className="px-2 py-0.5 text-sm rounded-full bg-[#3B82F6]/10 text-[#3B82F6]"
                            >
                              {keyword}
                            </span>
                          ))}
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg bg-[#2563EB]/10 text-[#2563EB] hover:bg-[#2563EB]/20 transition-colors"
                    >
                      <UserPlus size={20} />
                    </button>
                  </div>
        ...
        {showIntroModal && selectedConnection && (
          ...
          >
            <motion.div
              ...
              className="w-full max-h-[85vh] overflow-y-auto bg-[#FFFFFF] rounded-t-2xl border-t border-[#E2E8F0] p-6"
            >
              ...
              {introSent ? (
                ...
                >
                  <div className="w-16 h-16 rounded-full bg-[#10B981]/20 flex items-center justify-center mx-auto mb-4">
                    <MessageCircle size={32} className="text-[#10B981]" />
                  </div>
                  <h3 className="text-xl font-bold text-[#1A1A2E] mb-2">소개 요청 완료!</h3>
                ...
                <>
                  <h3 className="text-lg font-semibold text-[#1A1A2E] mb-4">소개 요청하기</h3>

                  {/* 소개 경로 */}
                  <div className="flex items-center justify-center gap-3 mb-6 p-4 rounded-lg bg-[#F8F9FA]">
                    <div className="text-center">
                      <Avatar src={user?.profileImage} name={user?.name || '나'} size="sm" />
                      <p className="text-sm text-[#64748B] mt-1">나</p>
                    </div>
                    <div className="flex-1 h-0.5 bg-[#2563EB]" />
                    <div className="text-center">
                      <Avatar src={targetUser.profileImage} name={targetUser.name} size="sm" />
                      <p className="text-sm text-[#64748B] mt-1">{targetUser.name}</p>
                    </div>
                    <div className="flex-1 h-0.5 bg-[#2563EB]" />
                    <div className="text-center">
                      <Avatar src={selectedConnection.profileImage} name={selectedConnection.name} size="sm" />
                      <p className="text-sm text-[#64748B] mt-1">{selectedConnection.name}</p>
                    </div>
                  </div>

                  {/* 소개받고 싶은 사람 정보 */}
                  <div className="p-4 rounded-lg bg-[#F8F9FA] border border-[#E2E8F0] mb-4">
                    <p className="text-base text-[#64748B] mb-2">소개받고 싶은 분</p>
                    <div className="flex items-center gap-3">
                      <Avatar src={selectedConnection.profileImage} name={selectedConnection.name} size="md" />
                      <div>
                        <p className="font-medium text-[#1A1A2E]">{selectedConnection.name}</p>
                        <p className="text-base text-[#64748B]">
                  ...
                  {/* 목적 선택 */}
                  <div className="mb-4">
                    <p className="text-base text-[#64748B] mb-2">소개 목적</p>
                        ...
                        <button
                          ...
                          className={`px-3 py-2 rounded-lg text-base transition-all ${
                            introPurpose === purpose.value
                              ? 'bg-[#2563EB]/20 text-[#2563EB] border border-[#2563EB]'
                              : 'bg-[#E2E8F0] text-[#64748B] border border-transparent'
                          }`}
                        >
                          {purpose.label}
                        </button>
                  ...
                  {/* 메시지 */}
                  <div className="mb-6">
                    <p className="text-base text-[#64748B] mb-2">소개 요청 메시지</p>
                    <textarea
                      value={introMessage}
                      onChange={(e) => setIntroMessage(e.target.value)}
                      placeholder={`${targetUser.name}님에게 ${selectedConnection.name}님을 왜 소개받고 싶은지 설명해주세요...`}
                      className="w-full h-32 p-4 rounded-xl bg-[#FFFFFF] border border-[#E2E8F0] text-[#1A1A2E] placeholder:text-[#94A3B8] resize-none focus:outline-none focus:border-[#2563EB]"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-xl bg-[#2563EB] text-white font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    소개 요청 보내기
                  </button>
                </>
